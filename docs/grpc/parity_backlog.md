...
| docling-serve `grpc-native-converter` | Content matches upstream tip (**yaml/vtt/html_split/dclx**, **S3.region**, Ray metrics, settings). Git ancestry still diverges (35 commits). A literal `merge upstream/main` is blocked by pre-push: upstream `#641` carries `Co-authored-by: Claude Opus…`. |
