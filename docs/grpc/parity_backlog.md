...
| docling-serve `grpc-native-converter` | Content matches upstream tip (**yaml/vtt/html_split/dclx**, **S3.region**, Ray metrics, settings). **`PROCESSING_PIPELINE_NATIVE` live** on slim≥2.127. Ancestry still diverges (merge blocked by upstream AI trailer on #641). |
