...
| gRParse `document_timeout` / `page_range` | **Landed** `9e3234c`: deadline cap + CV page span. |
...
4. PDF-collector / NATIVE path: apply `page_range` when not using CV scheduler.
5. Streaming `page_range` if DocumentChunk gains the field.
